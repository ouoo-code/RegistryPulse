...
const filtered = computed(() => sortHistoryNewest(results.value.filter(result => {
  ...
  return (!sourceFilter.value || result.source_id === sourceFilter.value)
    ...
    && (!query.value || `${sourceName} ${result.probe_mode || ''} ${result.error || ''} ${result.error_stage || ''}`.toLowerCase().includes(query.value.toLowerCase()))
})))
...
function probeModeLabel(mode: string | undefined) {
  switch (mode) {
    case 'registry': return t.value.registryProbe
    case 'manifest': return t.value.manifestProbe
    case 'http': return t.value.httpProbe
    case 'docker_pull': return t.value.dockerPullProbe
...
<template>
  ...
  <BaseTable class="history-admin-table" :empty="loading ? t.loading : (!paged.length ? t.noResults : '')">
    <template #head><thead><tr><th class="history-time-column">{{ t.checked }}</th><th class="history-source-column">{{ t.registry }}</th><th class="history-mode-column">{{ t.probeMode }}</th><th class="history-status-column">{{ t.status }}</th><th class="history-response-column">{{ t.response }}</th><th class="history-stage-column">{{ locale === 'zh' ? '错误阶段' : 'Error stage' }}</th><th class="history-error-column">{{ t.error }}</th></tr></thead></template>
    <template #body><tbody><tr v-for="result in paged" :key="`${result.source_id}-${result.checked_at}-${result.status}`"><td class="history-time-column">{{ formatDateTime(result.checked_at) || t.unknown }}</td><td class="history-source-column">{{ sourceNames.get(result.source_id) || result.source_id }}</td><td class="history-mode-column">{{ probeModeLabel(result.probe_mode) }}</td><td class="history-status-column"><span class="state" :class="result.status">{{ statusLabel(result.status, t) }}</span></td><td class="history-response-column">{{ result.response_ms ? `${result.response_ms} ms` : '—' }}</td><td class="history-stage-column">{{ result.error_stage || '—' }}</td><td class="history-error-cell history-error-column">{{ result.error || '—' }}</td></tr></tbody></template>
  </BaseTable>
