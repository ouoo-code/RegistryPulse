...
<template>
  ...
  <BaseDialog :open="dialogOpen" :title="editing ? t.edit : t.add" @close="dialogOpen = false"><form class="notification-form" @submit.prevent="save"><FormField :label="t.name"><input v-model="form.name" required></FormField><FormField :label="t.type"><select v-model="form.type"><option value="webhook">Webhook</option><option value="gotify">Gotify</option><option value="smtp">SMTP Email</option></select></FormField><FormField v-if="form.type !== 'smtp'" :label="t.url"><input v-model="form.url" type="url" required></FormField><FormField v-if="form.type === 'gotify'" label="Token"><input v-model="form.token" type="password" :placeholder="editing ? '******' : ''" :required="!editing"></FormField><FormField v-if="form.type === 'webhook'" label="Secret"><input v-model="form.secret" type="password" :placeholder="editing ? '******' : ''"></FormField><template v-if="form.type === 'smtp'"><FormField label="Host"><input v-model="form.host" required></FormField><FormField label="Port"><input v-model="form.port" required></FormField><FormField label="Username"><input v-model="form.username"></FormField><FormField label="Password"><input v-model="form.password" type="password" :placeholder="editing ? '******' : ''"></FormField><FormField label="From"><input v-model="form.from" type="email" required></FormField><FormField label="To"><input v-model="form.to" type="email" required></FormField></template><label class="checkbox-field"><input v-model="form.enabled" type="checkbox">{{ t.enabled }}</label><div class="editor-form-actions"><button class="icon-button" type="button" @click="dialogOpen = false">{{ t.cancel }}</button><button class="icon-button" type="submit">{{ t.save }}</button></div></form></BaseDialog>
</template>
